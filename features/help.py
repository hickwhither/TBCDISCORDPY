...
class CustomHelp(commands.HelpCommand):
    ...
    async def send_bot_help(self, mapping):
        ...
        for cog, cmds in mapping.items():
            ...
            cmd_list = "\n".join(
                f"`{prefix}{c.name}` — {c.short_doc or 'No description'}" for c in cmds
            )
            embed.add_field(
                name=cog.qualified_name if cog else "Misc", value=cmd_list, inline=False
            )
    ...
    async def send_cog_help(self, cog):
        ...
        embed.set_footer(
            text=f'Type "{self.context.prefix}help <command>" for more info.'
        )
        await self.get_destination().send(embed=embed)
